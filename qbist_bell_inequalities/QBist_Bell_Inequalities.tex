...
\end{align}
This is quite general: a Bell inequality always takes the form
\begin{align}
\B{c}^T\B{p}\leq \beta,	
\end{align}
...
This result opens up a number of important research directions. In particular, one may ask: optimizing over number of parties, reference devices, initial state, choices of measurement, and Bell inequality, which scenario gives the largest bound? Is it possible to find a bound which is equal to the actual value of $\lVert  \Phi - I\rVert$? Fixing a reference device, which Bell scenario gives the largest bound? Fixing a Bell scenario, which reference device gives the largest bound? And finally, can we implement such scenarios efficiently on a quantum computer?
